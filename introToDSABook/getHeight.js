...
function getHeight(tree) {
  function climbTree(branch, steps) {
    let right = branch.right ? climbTree(branch.right, steps + 1) : steps;
    let left = branch.left ? climbTree(branch.left, steps + 1) : steps;
    return Math.max(right, left)
  }
  return climbTree(tree, 1);
}
