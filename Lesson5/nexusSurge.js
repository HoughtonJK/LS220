...
function nexusSurge(actions) {
  ...
  while(actions.length > 0) {
    ...
    switch (action) {
      case '+':
        let val2 = scoreboard.pop();
        let val1 = scoreboard.pop();
        scoreboard.push(val1);
        scoreboard.push(val2);
        scoreboard.push(val1 + val2);
        break;
      case '*':
        let val = scoreboard.pop();
        scoreboard.push(val);
        scoreboard.push(val * 2);
        break;
      case '-':
        scoreboard.pop();
        break;
  ...
  let result = scoreboard.reduce((sum, score) => sum + score, 0);
  return result;
}
