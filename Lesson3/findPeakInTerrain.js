...
function findPeakInTerrain(terrain) {
  function climbToPeak(front, end) {
    let middle = Math.floor((front + end) / 2);
    if(terrain[middle] < terrain[middle - 1]) {
      return climbToPeak(front, middle - 1);
    } else if(terrain[middle] < terrain[middle + 1]) {
      return climbToPeak(middle + 1, end);
    } else {
      return middle;
    }
  }
  return climbToPeak(0, terrain.length - 1);
}
