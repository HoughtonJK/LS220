...
// Version using Binary Tree O(N * logS)
/**/
function findTruckCapacity(orderVolumes, maxTrips) {
  let totalCapacity = orderVolumes.reduce((sum, order) => sum + order, 0);
  let low = 0;
  let high = totalCapacity;
  let mid = 0;
  while(low < high) {
  ...
  console.log(maxTrips, mid + 1);
  return mid + 1;
}

/*/
//Version with pointers O(N * T)
function findTruckCapacity(orderVolumes, maxTrips) {
